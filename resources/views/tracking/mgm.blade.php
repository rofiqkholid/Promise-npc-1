...
@section('title', $pageTitle ?? 'Persetujuan Management')
@section('page_title', 'Transaksi / ' . ($pageTitle ?? 'Persetujuan Management'))

@section('content')
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
        <h2 class="text-xl font-semibold text-gray-800 dark:text-white flex items-center gap-2">
            <i class="fa-solid {{ $pageIcon ?? 'fa-user-tie' }} text-blue-500"></i> {{ $pageTitle ?? 'Persetujuan Management' }}
        </h2>
        @if(isset($pageDesc))
        ...
        <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
            <table class="w-full text-sm text-left text-slate-600 dark:text-slate-400">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="bg-white dark:bg-gray-800 border-b dark:border-gray-700 hover:bg-blue-50/50 dark:hover:bg-gray-700/30 transition text-sm {{ $part->status !== 'WAITING_MGM_CHECK' ? 'opacity-[0.65] grayscale-[0.3] pointer-events-none' : '' }}">
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-center align-middle">
                            @if(in_array($part->status, ['PO_REGISTERED', 'WAITING_DEPT_CONFIRM', 'WAITING_QE_CHECK']))
                                <div class="inline-flex flex-col items-center gap-1.5 px-3 py-2 rounded bg-slate-50 border border-slate-200 text-[10px] text-slate-500 italic">
                                    <i class="fa-solid fa-microscope text-sm"></i> Sedang Pengecekan QC
                                </div>
                            @else
                        ...
                        <td class="px-6 py-4 text-right align-middle pointer-events-auto">
                            ...
                            @elseif($part->status === 'WAITING_MGM_CHECK')
                                <a href="{{ route('checksheets.create', $part->id) }}" class="inline-flex px-4 py-2 bg-purple-500 hover:bg-purple-600 text-white rounded shadow-sm font-bold transition items-center gap-2 text-[11px]" style="background-color: #a855f7;">
                                    <i class="fa-solid fa-user-check"></i> Form MGM Approve
                                </a>
