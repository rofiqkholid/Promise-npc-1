...
@section('title', 'Add New STD Part')
@section('page_title', 'Master Data / STD Parts / Add')
...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 max-w-2xl mx-auto">
    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
        <h2 class="text-lg font-semibold text-gray-800 dark:text-white flex items-center gap-2">
            <i class="fa-solid fa-plus text-blue-500"></i> Create New STD Part
        </h2>
    </div>
    <form action="{{ route('master.std-parts.store') }}" method="POST">
        @csrf
        <div class="p-6 space-y-5">
            <div>
                ...
                <input type="text" id="name" name="name" class="w-full text-sm border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-white" value="{{ old('name') }}" placeholder="e.g. BOLT, BRAKE PIPE" required>
                @error('name')
            ...
            <div class="flex items-center gap-2 pt-2">
                <input type="checkbox" id="is_active" name="is_active" value="1" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" checked>
                <label for="is_active" class="text-sm font-medium text-gray-700 dark:text-gray-300">Set as Active Status</label>
            </div>
        </div>
