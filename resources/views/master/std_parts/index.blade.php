...
<div x-data="{ showImportModal: false }">
    <div class="bg-white dark:bg-gray-800 shadow border border-gray-200 dark:border-gray-700">
        <div class="px-4 py-2 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
            ...
            <div class="flex gap-2">
                <button type="button" @click="showImportModal = true" class="px-4 py-2 bg-white text-gray-700 dark:bg-gray-700 dark:text-gray-200 border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600 transition shadow-sm font-medium text-[13px] flex items-center gap-2 rounded-none">
                    <i class="fa-solid fa-file-excel text-green-600"></i> Import Excel
                </button>
                <a href="{{ route('master.std-parts.create') }}" class="px-4 py-2 bg-gradient-to-r from-blue-600 to-cyan-600 text-white hover:from-blue-700 hover:to-cyan-700 transition shadow-md shadow-blue-500/20 font-medium text-[13px] flex items-center gap-2 rounded-none">
        ...
        <div class="overflow-x-auto border-t border-gray-200 dark:border-gray-700">
            <table id="stdPartsTable" class="w-full text-sm text-left text-slate-600 dark:text-slate-400">
                <thead class="bg-gray-100 dark:bg-gray-700/50 text-slate-800 dark:text-slate-200 border-b border-gray-200 dark:border-gray-600 uppercase text-xs tracking-wider">
                    <tr>
                        <th scope="col" class="px-4 py-2 font-semibold w-16 text-center">No</th>
                        <th scope="col" class="px-4 py-2 font-semibold">Name</th>
                        <th scope="col" class="px-4 py-2 font-semibold w-24">Status</th>
                        <th scope="col" class="px-4 py-2 font-semibold text-right w-24">Action</th>
                    </tr>
                </thead>
                <tbody>
    ...
    <div x-show="showImportModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" x-cloak style="display: none;">
        <div x-show="showImportModal" @click.away="showImportModal = false" x-transition class="bg-white dark:bg-gray-800 w-full max-w-md shadow-xl border border-gray-200 dark:border-gray-700">
            <div class="px-4 py-2 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-800/50">
            ...
            <div class="p-6">
                ...
                <form action="{{ route('master.std-parts.import.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="flex justify-end gap-2 pt-4 border-t border-gray-100 dark:border-gray-700">
                        <button type="button" @click="showImportModal = false" class="px-4 py-2 bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600 transition text-[13px] rounded-none">Cancel</button>
                        <button type="submit" class="px-4 py-2 bg-blue-600 text-white hover:bg-blue-700 transition text-[13px] flex items-center gap-2 rounded-none">
...
<script>
    $(document).ready(function() {
        initPromiseDataTable('#stdPartsTable', {
            ...
            columns: [
                { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false, className: 'px-4 py-2 font-bold text-center text-indigo-600 dark:text-indigo-400' },
                { data: 'name', name: 'name', className: 'px-4 py-2 font-semibold text-slate-900 dark:text-white' },
                { data: 'is_active', name: 'is_active', className: 'px-4 py-2', searchable: false },
