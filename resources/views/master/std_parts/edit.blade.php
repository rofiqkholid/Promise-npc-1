    ...
    <form action="{{ route('master.std-parts.update', $std_part->hashed_id) }}" method="POST">
        ...
        <div class="p-6 space-y-5">
            <div>
                ...
                <input type="text" id="name" name="name" class="w-full text-sm border-gray-300 dark:border-gray-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:text-white" value="{{ old('name', $std_part->name) }}" required>
                @error('name')
            ...
            <div class="flex items-center gap-2 pt-2">
                <input type="checkbox" id="is_active" name="is_active" value="1" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" {{ $std_part->is_active ? 'checked' : '' }}>
