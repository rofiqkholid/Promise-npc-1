...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700">
    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
        <h2 class="text-lg font-semibold text-gray-800 dark:text-white flex items-center gap-2">
            <i class="fa-solid fa-truck-fast text-blue-500"></i> Daftar Tujuan Pengiriman
        </h2>
            ...
            <table class="w-full text-sm text-left text-slate-600 dark:text-slate-400">
                <thead class="bg-gray-100 dark:bg-gray-700/50 text-slate-800 dark:text-slate-200 border-b border-gray-200 dark:border-gray-600 uppercase text-xs tracking-wider">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-4 font-semibold">Nama Tujuan (Target Delivery)</th>
                ...
                <tbody>
                    ...
                    <tr class="bg-white dark:bg-gray-800 border-b dark:border-gray-700 hover:bg-blue-50/50 dark:hover:bg-gray-700/30 transition group">
                        ...
                        <td class="px-6 py-4 font-bold text-slate-900 dark:text-white">{{ $target->target_name }}</td>
                        <td class="px-6 py-4">
                            @if($target->is_active)
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-1 opacity-50 group-hover:opacity-100 transition">
                                ...
                                <form action="{{ route('master.delivery-targets.destroy', $target->id) }}" method="POST" class="inline" onsubmit="return confirm('Hapus tujuan pengiriman ini secara permanen?');">
                                    @csrf
                                    @method('DELETE')
