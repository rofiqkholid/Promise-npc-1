            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 pb-8 border-b border-gray-200 dark:border-gray-700">
                ...
                <div>
                    ...
                    <input type="text" name="code" id="code" value="{{ old('code', $role->code) }}" required {{ $role->code === 'admin' ? 'readonly' : '' }}
                        class="mt-1 w-full border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm {{ $role->code === 'admin' ? 'bg-gray-100 cursor-not-allowed' : '' }}">
                    @error('code')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
